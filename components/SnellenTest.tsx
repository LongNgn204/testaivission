...
import { ArrowUpIcon, ArrowRightIcon, ArrowDownIcon, ArrowLeftIcon, RotateCcwIcon, DownloadIcon, ShareIcon, BrainCircuitIcon } from './ui/Icons';
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-4">
        <button onClick={() => window.location.reload()} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors"><RotateCcwIcon size={18} />{t('redo_test')}</button>
        <button
...
const TestScreen: React.FC<{ question: SnellenQuestion; handleAnswer: (rotation: number) => void; }> = ({ question, handleAnswer }) => {
  ...
  const directionButtons = [
    { Icon: ArrowRightIcon, labelKey: 'direction_right', rotation: 0 },    // E opens RIGHT → 0°
    { Icon: ArrowDownIcon, labelKey: 'direction_down', rotation: 90 },     // E opens DOWN → 90°
    { Icon: ArrowLeftIcon, labelKey: 'direction_left', rotation: 180 },    // E opens LEFT → 180°
    { Icon: ArrowUpIcon, labelKey: 'direction_up', rotation: 270 }         // E opens UP → 270°
  ];
