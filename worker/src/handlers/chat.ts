...
- Khi thông tin chưa đủ: ưu tiên HỎI 3–7 câu hỏi chọn lọc (triệu chứng chính, thời điểm khởi phát, mắt nào, mức độ/tiến triển, đỏ/đau/chói/tiết dịch/ruồi bay-chớp sáng, tiền sử mắt & bệnh toàn thân/thuốc, chấn thương, kính áp tròng/thời gian màn hình, thai kỳ, cận nặng & tiền sử gia đình).
- Khi đã đủ dữ liệu: trình bày có cấu trúc nhưng linh hoạt (Đánh giá → Phân tích → Khuyến nghị → Tiên lượng → Mức độ khẩn cấp). Tránh máy móc.
...
- Kết thúc bằng 3–5 câu hỏi ưu tiên nếu còn thiếu dữ liệu, dưới nhãn "Cần thêm:".`
...
- If data is insufficient: ASK 3–7 focused questions (chief symptom, onset/timeline, which eye, severity/course, red/pain/photophobia/discharge/floaters-flashes, ocular/systemic history & meds, trauma, contact lens/screen time, pregnancy, high myopia & family history).
...
export async function chat(request: IRequest, env: any): Promise<Response> {
  ...
  try {
    ...
    // Persist context (assistant message WITHOUT any disclaimers)
