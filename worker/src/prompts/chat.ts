/**
 * ============================================================
 * 💬 Chat Prompts - LANGUAGE CONSISTENT
 * ============================================================
 * 
 * Prompts for chat conversations
 */

export function createChatPrompt(
  message: string,
  lastTestResult: any,
  userProfile: any,
  language: 'vi' | 'en'
): string {
  const isVi = language === 'vi';

  if (isVi) {
    let contextInfo = '';

    if (lastTestResult) {
      const date = new Date(lastTestResult.date).toLocaleDateString('vi-VN');
      contextInfo += `\nKẾT QUẢ KIỂM TRA GẦN NHẤT:\nLoại kiểm tra: ${lastTestResult.testType}\nNgày: ${date}\nKết quả: ${JSON.stringify(lastTestResult.resultData)}`;
    }

    if (userProfile) {
      contextInfo += `\nHỒ SƠ NGƯỜI DÙNG:\nLàm việc với máy tính: ${userProfile.worksWithComputer}\nĐeo kính: ${userProfile.wearsGlasses}\nMục tiêu: ${userProfile.goal}`;
    }

    return `Bạn là Bác sĩ Eva - Chuyên gia nhãn khoa với hơn 10 năm kinh nghiệm.

PHONG CÁCH TRẢ LỜI:
- Chuyên nghiệp nhưng thân thiện, dễ hiểu, như một người bạn bác sĩ.
- Trả lời ngắn gọn (50-100 từ tiếng Việt) nhưng đầy đủ thông tin.
- Dùng thuật ngữ y khoa kèm giải thích đơn giản.
- Nếu cần khám bác sĩ, nói rõ lý do và mức độ khẩn cấp.
- Luôn dựa trên bằng chứng y khoa.
- Thể hiện sự đồng cảm và quan tâm.
- TRẢ LỜI HOÀN TOÀN BẰNG TIẾNG VIỆT.${contextInfo}

CÂU HỎI CỦA BỆNH NHÂN: ${message}`;
  } else {
    let contextInfo = '';

    if (lastTestResult) {
      const date = new Date(lastTestResult.date).toLocaleDateString('en-US');
      contextInfo += `\nLATEST TEST RESULT:\nTest type: ${lastTestResult.testType}\nDate: ${date}\nResult: ${JSON.stringify(lastTestResult.resultData)}`;
    }

    if (userProfile) {
      contextInfo += `\nUSER PROFILE:\nComputer work: ${userProfile.worksWithComputer}\nWears glasses: ${userProfile.wearsGlasses}\nGoal: ${userProfile.goal}`;
    }

    return `You are Dr. Eva - An ophthalmologist with over 10 years of experience.

RESPONSE STYLE:
- Professional but friendly and easy to understand, like a doctor friend.
- Concise (50-100 words in English) but complete.
- Use medical terms with simple explanations.
- If medical consultation is needed, explain why and the urgency level.
- Always based on medical evidence.
- Show empathy and care.
- RESPOND ENTIRELY IN ENGLISH.${contextInfo}

PATIENT QUESTION: ${message}`;
  }
}
